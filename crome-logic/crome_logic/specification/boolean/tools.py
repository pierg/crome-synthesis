...
def combine_atoms(atoms: list[Node], operation: str) -> str:
    if operation == "and":
        ...
        return implies_(
            list(atoms)[0].attr["label"],
            list(atoms)[1].attr["label"])
    elif operation == "not":
...
def convert_dict_to_spot_string(operation_graph: dict) -> str:
    ...
    if key is not None:
        ...
        if len(operation_graph.keys()) == 1:
            return combine_atoms(atoms=operation_graph[key],
                                 operation=key.attr["label"])
        # print(
        #     f"SELECTED\n{key.attr['label']} -> {[elem.attr['label'] for elem in operation_graph[key]]}"
        # )
        new_label = combine_atoms(atoms=operation_graph[key],
                                  operation=key.attr["label"])
